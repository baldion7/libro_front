export const PostActivityProyect = [
  {
    id: 1,
    imagen: [{
      id: 1,
      img: 'src/img/IMG_20230613_165456-1-461x1024.jpg',
    },
      {
        id: 2,
        img: 'src/img/IMG_20230613_170051-461x1024.jpg',

      },
      {
        id: 3,
        img: 'src/img/IMG_20230613_162921-461x1024.jpg',

      },

    ],
    title: ' Entrevista Radial con los Abuelos en la Emisora Comunitaria San Juan de Girón 88.2 FM',
    text: 'En una tarde cargada de nostalgia y aprendizaje, los estudiantes se convirtieron en entrevistadores en el programa «sorbitos de Memoria» en la emisora comunitaria San Juan de Girón 88.2 FM. Bajo la dirección de la historiadora Patricia San Miguel, esta actividad tuvo lugar el día martes a las 4:50 p.m. y fue una oportunidad para que los abuelos compartieran sus recuerdos y evocaran el pasado.\n' +
      '\n' +
      'Durante la entrevista, los estudiantes guiaron a sus abuelos en un viaje a través del tiempo, invitándolos a rememorar las costumbres, fiestas, juguetes y cómo la tecnología ha interactuado en sus vidas a lo largo de los años. Fue una experiencia fascinante para los entrevistadores y una oportunidad invaluable para capturar y preservar la memoria colectiva de sus abuelos.\n' +
      '\n' +
      'Los abuelos, con una mezcla de nostalgia y entusiasmo, compartieron sus vivencias y experiencias, transportando a los oyentes a épocas pasadas. Desde descripciones detalladas de celebraciones tradicionales hasta relatos de juegos y juguetes populares, cada historia tejía una imagen vívida de cómo era la vida en tiempos pasados.\n' +
      '\n' +
      'Además, los abuelos también compartieron cómo la tecnología ha influido en sus vidas y en la sociedad en general. Comentaron sobre los cambios que han experimentado con la llegada de la tecnología, cómo ha mejorado la comunicación y cómo ha impactado diferentes aspectos de sus rutinas diarias.\n' +
      '\n' +
      'Esta actividad permitió a los estudiantes conectarse de manera profunda con sus raíces, descubrir nuevas perspectivas y apreciar el valor de la tradición y el progreso. Los abuelos, a su vez, se sintieron valorados y escuchados, experimentando la satisfacción de compartir su sabiduría con las generaciones más jóvenes.\n' +
      '\n' +
      'En resumen, «Sorbitos de Memoria» en la emisora comunitaria San Juan de Girón 88.2 FM brindó un espacio enriquecedor en el que los estudiantes entrevistadores se sumergieron en las historias de sus abuelos, explorando las costumbres, fiestas, juguetes y el impacto de la tecnología en sus vidas. Esta actividad fomentó la conexión intergeneracional y promovió la preservación de la memoria colectiva, enriqueciendo la comprensión de la historia y la cultura de la comunidad.',
    slider: [{
      url: 'src/img/IMG_20230613_162556-1024x461.jpg',
    },
      {
        url: 'src/img/IMG_20230613_163742-1024x461.jpg',
      },
      {
        url: 'src/img/IMG_20230613_163629-1024x461.jpg',
      },
      {
        url: 'src/img/WhatsApp Image 2023-07-25 at 7.11.52 PM.jpeg',
      }
    ],
    youtube: '4sZ6uOFaVc8'

  },
  {
    id: 2,
    imagen: [{
      id: 1,
      img: 'src/img/IMG_20230702_180715-454x1024.jpg',
    },
      {
        id: 2,
        img: 'src/img/IMG_20230621_094610-edited-scaled.jpg',

      },
      {
        id: 3,
        img: 'src/img/IMG_20230702_180742-461x1024.jpg',

      }
    ],
    title: 'Queridos Abuelos: Una Carta Cautivadora de Amor, Agradecimiento y Sabiduría',
    text: 'En este inspirador ejercicio de escritura, los estudiantes tuvieron la oportunidad de expresar su amor, gratitud y aprecio hacia sus abuelos a través de una carta. Fue un momento especial para reflexionar sobre la importancia de la figura de los abuelos en sus vidas y compartir con ellos pensamientos y sentimientos sinceros.\n' +
      '\n' +
      'Durante este ejercicio, los estudiantes se sumergieron en sus recuerdos, reviviendo momentos especiales junto a sus abuelos y reconociendo el impacto positivo que han tenido en su desarrollo personal. Fue una ocasión para apreciar la sabiduría acumulada a lo largo de los años y para expresar cómo los abuelos han dejado huellas imborrables en sus corazones.\n' +
      '\n' +
      'A través de sus palabras, los estudiantes transmitieron gratitud por los valores transmitidos por sus abuelos, por el amor incondicional recibido y por el tiempo y la atención dedicados a ellos en estas actividades realizadas en este propuesta educativa como lo es pisquitas de memorias. Además, aprovecharon la oportunidad para expresar la importancia de la conexión intergeneracional y cómo los abuelos han dejado una profunda impresión en su identidad y forma de ver el mundo.\n' +
      '\n' +
      'En estas cartas, los estudiantes compartieron momentos especiales, anécdotas y lecciones aprendidas de sus abuelos. Expresaron cómo los abuelos han sido un apoyo constante, un ejemplo a seguir y una fuente de sabiduría invaluable. A través de sus palabras, los estudiantes demostraron la importancia de mantener el vínculo con los abuelos y de valorar su presencia en sus vidas.\n' +
      '\n' +
      'En resumen, este ejercicio de escritura fue una hermosa oportunidad para que los estudiantes expresaran sus sentimientos hacia sus abuelos. A través de sus cartas, compartieron amor, gratitud y aprecio, reconociendo la influencia positiva de sus abuelos en sus vidas. Este ejercicio resaltó la importancia de mantener vivos los lazos familiares y el valor de aprender de las generaciones anteriores.',
    slider: [{
      url: 'src/img/IMG_20230702_180838-779x1024.jpg',
    },
      {
        url: 'src/img/IMG_20230702_180919-1024x492.jpg',
      }
    ],
  },
  {
    id: 3,
    imagen: [{
      id: 1,
      img: 'src/img/IMG_20230702_093822-900x1024.jpg',
    },
      {
        id: 2,
        img: 'src/img/IMG_20230702_093338-495x1024.jpg',

      },
      {
        id: 3,
        img: 'src/img/IMG_20230702_093312-460x1024.jpg',

      }
    ],
    title: '¿Para qué sirve una niña o un niño? ¡Descubre el asombroso propósito detrás de su existencia, en respuestas dadas por los estudiantes!',
    text: 'En esta fascinante actividad, se da la oportunidad de plantear la pregunta: «¿Para qué sirve una niña o un niño?» y escuchar las respuestas de los propios niños. Fue emocionante ver cómo expresaron sus saberes y percepciones, analizando y viendo la realidad desde su perspectiva única.\n' +
      '\n' +
      'Los niños, con su curiosidad innata y su capacidad de reflexionar, nos sorprendieron con respuestas genuinas y llenas de sabiduría. A través de su sentido y su conocimiento de la vida cotidiana, lograron articular sus respuestas, brindándonos una visión auténtica y fresca sobre el propósito de una niña o un niño en el mundo.\n' +
      '\n' +
      'Cada niño aportó su propia interpretación, influenciada por sus experiencias, educación y entorno. Sus respuestas revelaron la profundidad de su pensamiento y su capacidad para analizar la realidad que les rodea. Fue conmovedor presenciar cómo reflexionaron sobre el valor y el significado de su existencia.\n' +
      '\n' +
      'A través de sus respuestas, los niños demostraron que una niña o un niño sirve para mucho más que simplemente crecer y ocupar un lugar en la sociedad. Describieron su capacidad para iluminar el mundo con su inocencia y alegría, su habilidad para amar incondicionalmente, su papel como futuros constructores de un mundo mejor, su deseo de aprender y explorar, su capacidad para unir a las familias y fortalecer los lazos afectivos, su contribución a la diversidad y la individualidad, su inspiración para la creatividad y los sueños, su recordatorio de cuidar y proteger el entorno, su resiliencia para superar obstáculos y, en última instancia, su capacidad para hacernos mejores personas.\n' +
      '\n' +
      'Estas respuestas, enriquecidas por la autenticidad y la sabiduría de los niños, nos invitan a reflexionar sobre nuestro propio papel en la vida y cómo podemos nutrir y apoyar el crecimiento de las nuevas generaciones. Nos recuerdan que los niños son seres extraordinarios, capaces de ver la realidad de una manera única y brindarnos una perspectiva fresca y valiosa.\n' +
      '\n' +
      'En resumen, esta actividad permite sumergirnos en la visión de los niños sobre el propósito de una niña o un niño en el mundo. Sus respuestas, expresadas a través de su sentido y su capacidad de análisis, nos recordaron la importancia de escuchar y valorar las voces de los más jóvenes, reconociendo su sabiduría innata y su capacidad para ver la realidad con una claridad sorprendente.',
    slider: [{
      url: 'src/img/IMG_20230702_093249-461x1024.jpg',
    },
      {
        url: 'src/img/IMG_20230702_093230-461x1024.jpg',
      },
      {
        url: 'src/img/IMG_20230702_093055-461x1024.jpg',
      }

    ],

  },
  {
    id: 4,
    imagen: [{
      id: 1,
      img: 'src/img/IMG_20230702_093822-900x1024.jpg',
    },
      {
        id: 2,
        img: 'src/img/IMG_20230702_093338-495x1024.jpg',

      },
      {
        id: 3,
        img: 'src/img/IMG_20230702_093312-460x1024.jpg',

      }
    ],
    title: 'Inmersión cultural: Visita de estudiantes de la comunidad indígena Inga del Putumayo a la sede educativa',
    text: 'En esta emocionante actividad, los estudiantes tuvieron la maravillosa oportunidad de recibir la visita de jóvenes estudiantes de la UIS de la comunidad indígena Inga en la sede educativa. Durante esta visita, los integrantes de la comunidad nos recordaron nuestras raíces y nos sumergieron en su rica cultura ancestral. Ubicada en el hermoso departamento del Putumayo, la comunidad Inga nos brindó una visión completa de su economía, vivienda, educación, costumbres y tradiciones.\n' +
      '\n' +
      'Desde el momento en que llegaron, nos hace una exposición de sus instrumentos musicales, sus artesanías collares. Los miembros de la comunidad nos hablaron sobre la importancia de preservar sus tradiciones y transmitirlas a las generaciones futuras, la importancia que tienen en su cosmovisión y cómo se vinculan con la naturaleza que les rodea. Nos enseñaron acerca de la estructura política organizada de la comunidad, destacando la «Llagta tandari» o Asamblea General, el «Atun tambu iuai» o Cabildo Menor, el «Uchulla tambukuna» o Cabildo Menor y los «Wasikamakuna» o Guardia Indígena.\n' +
      '\n' +
      'Uno de los aspectos más fascinantes de esta experiencia fue aprender sobre el idioma Inga. Nos enseñaron algunas palabras y frases básicas, y nos maravillamos con la riqueza de su vocabulario. También descubrimos los diferentes atuendos tradicionales utilizados por las mujeres, conocidos como «pachas», y por los hombres, llamados «kusmas».\n' +
      '\n' +
      'Durante la visita, se presenció la explicación de sus ceremonias y festividades religiosas, en las cuales los miembros de la comunidad demuestran una profunda conexión con la naturaleza. Nos enseñaron canciones y danzas tradicionales, acompañadas de instrumentos autóctonos. Fue realmente inspirador ver en un documental en «Los informantes» cómo los niños Inga reciben entrenamiento en la Guardia Indígena desde temprana edad, aprendiendo la importancia de proteger su tierra y sus tradiciones.\n' +
      '\n' +
      'Además, nos introdujeron al mundo de los animales en su lengua nativa. Aprendimos palabras como «alku» para perro, «misitu» para gato y «atawalpa» para gallina. También disfrutamos de canciones infantiles, como la famosa «Vaca Lola», que nos permitieron compartir risas y momentos divertidos con estos jóvenes estudiantes de la comunidad Inga.\n' +
      '\n' +
      'En resumen, esta visita de los jóvenes estudiantes indígenas Inga del Putumayo fue una experiencia enriquecedora y reveladora. A través de sus relatos, su música, su vestimenta y su profundo respeto por la naturaleza, nos transmitieron una invaluable lección sobre la importancia de preservar nuestras raíces y valorar la diversidad cultural.\n' +
      '\n' +
      'Debemos mirar con reverencia el pasado de nuestros ancestros y nutrir un profundo respeto por nuestros aborígenes, ya que ellos son la cuna de la civilización de nuestro hermoso país.\n' +
      '\n',
    slider: [{
      url: 'src/img/IMG_20230607_092929-1024x461.jpg',
    },
      {
        url: 'src/img/IMG_20230607_081333-1-1024x461.jpg',
      },
      {
        url: 'src/img/IMG_20230607_093139-1024x461.jpg',
      }

    ],
    youtube: 'f1ec6tgzBR0',
    videos: [
      {
        video: 'src/video/VID-20230620-WA0031.mp4'
      },
      {
        video: 'src/video/VID-20230620-WA0034.mp4'
      }
    ]
  },
  {
    id: 5,
    imagen: [{
      id: 1,
      img: 'src/img/IMG_20230621_100821-edited-461x1024.jpg',
    },
      {
        id: 2,
        img: 'src/img/IMG_20230702_012648-516x1024.jpg',

      },
      {
        id: 3,
        img: 'src/img/IMG_20230621_101100-461x1024.jpg',

      }
    ],
    title: '¡Peludos, Patosos y Con Cola! La Divertida Historia de Mi Mascota',
    text: 'La Historia de Mi Mascota» en formato de minilibro! En estas encantadoras páginas, los estudiantes presentan una aventura llena de risas, travesuras y momentos inolvidables junto a su querida mascota.\n' +
      '\n' +
      'Desde el momento en que la mascota llegó a su vida, entienden que estan destinados a ser los mejores amigos. En este minilibro, comparten cómo se conocieron y cómo han crecido juntos, superando desafíos y creando recuerdos que atesoraran para siempre.\n' +
      '\n' +
      'A lo largo de las páginas, cuentan anécdotas cómicas que les han ocurrido, como cuando la mascota dedicó a perseguir una mariposa y terminó tropezando con su propia cola. También revelan los talentos ocultos de su peludo compañero, como su habilidad para robar comida sin que nadie se dé cuenta.\n' +
      '\n' +
      'Pero no todo es diversión y travesuras. En este minilibro, también comparten momentos tiernos y emocionales. Ellos cuentan cómo su mascota ha sido su confidente en momentos de tristeza y cómo su lealtad incondicional los ha reconfortado en los días difíciles.\n' +
      '\n' +
      'A medida que avancemos en la historia, Estos pequeños escritores presentan a los personajes especiales que han conocido en su camino: desde sus amigos de cuatro patas hasta los vecinos curiosos que siempre se detuvieron a saludarnos durante sus paseos.\n' +
      '\n' +
      'A través de ilustraciones juguetonas y palabras llenas de cariño, este minilibro capturara la esencia de una relación única y especial. Será sus testimonios de la increíble conexión que pueden tener con sus amigos peludos y cómo su presencia puede llenar sus vidas de amor y felicidad.\n' +
      '\n' +
      'Así que prepárense para embarcarse en esta divertida y conmovedora aventura a través de «La Historia de Mi Mascota». ¡Es hora de abrir las páginas y dejarse llevar por las travesuras, las caricias y las lamidas afectuosas de su fiel compañero de patitas!',

  },
  {
    id: 6,
    title: '¡Queridos Taitas, No Se Asusten! Una Carta Llena de Travesuras y Confesiones',
    text: 'Damos inicio a un viaje lleno de amor y confidencias en nuestras cartas a los Taitas o los Papás! En esta hermosa actividad, los estudiantes tienen la oportunidad de escribir cartas muy especiales, en las cuales comparten algo que solo ellos saben, descubren emociones fuertes, alegrías y tristezas, y expresan sus más profundos sentimientos a sus queridos Taitas o Papás.\n' +
      '\n' +
      'En estas cartas íntimas, los estudiantes se sumergirán en un mundo de complicidad y confianza, donde podrán compartir secretos y revelar aspectos de su vida que solo ellos conocen. Cada carta será un tesoro cargado de emociones genuinas y palabras sinceras.\n' +
      '\n' +
      'A través de estas cartas, los estudiantes podrán contarles a sus Taitas algo que han descubierto, ya sea una nueva pasión, una habilidad oculta o incluso un hallazgo sorprendente que los ha dejado boquiabiertos. un momento de asombro y orgullo, en el que los estudiantes compartirán con sus seres queridos esas pequeñas victorias y descubrirán que los hacen sentir vivos.\n' +
      '\n' +
      'Pero también habrá espacio para expresar alegrías y tristezas, ya que todos atravesamos momentos de felicidad y desafíos en nuestras vidas. Los estudiantes tendrán la oportunidad de abrir sus corazones y contarles a sus Taitas o Papás ​​qué los hace felices y qué los entristece, creando un vínculo aún más fuerte y una comprensión profunda entre ellos.\n' +
      '\n' +
      'En estas cartas llenas de amor y sinceridad, los estudiantes encontrarán un espacio seguro para expresar todo lo que quieren transmitir a sus Taitas o Papás. Será una experiencia enriquecedora y conmovedora, tanto para los escritores como para aquellos que recibirán estas cartas llenas de amor y confianza.\n' +
      '\n' +
      'Así que prepárense para embarcarse en un viaje emocional y lleno de complicidad, donde las palabras se cambiaron en puentes que unen corazones y las cartas se cambiaron en testimonios de amor incondicional. ¡Es hora de abrir los sobres y descubrir los tesoros que se esconden en las cartas a los Taitas o los Papás!'
  },
  {
    id: 7,
    imagen: [{
      id: 1,
      img: 'src/img/IMG_20230701_222706-461x1024.jpg',
    },
      {
        id: 2,
        img: 'src/img/IMG_20230701_222803-461x1024.jpg',

      },
      {
        id: 3,
        img: 'src/img/IMG_20230701_222744-461x1024.jpg',

      }
    ],
    title: 'El Enigma de la Tarea Mentirosa: Entre Ficción y Realidad',
    text: 'Queridos estudiantes de 4-1JM.\n' +
      '\n' +
      'Con la presentación de esta actividad de aprendizaje creativa y llena de humor llamada «La Tarea Mentirosa». En esta divertido ejercicio de escritura, los estudiantes tienen la oportunidad de dejar volar su imaginación y presentar una excusa fantástica y chistosa sobre por qué no realizaron la tarea facilitada.\n' +
      '\n' +
      'El objetivo de esta actividad no es simplemente evadir responsabilidades, sino estimular la creatividad, el ingenio y el sentido del humor de los estudiantes. Les brindamos la oportunidad de convertir una situación común, como no hacer la tarea, en una historia fascinante y divertida que sorprenderá a todos.\n' +
      '\n' +
      'Cada estudiante tendrá la oportunidad de elaborar una excusa fantástica y completamente inventada, que explique de manera ingeniosa y entretenida por qué no podrá cumplir con la tarea. Pueden incluir elementos de magia, personajes extravagantes o situaciones absurdas que desafíen la lógica.\n' +
      '\n' +
      'El objetivo principal es fomentar la creatividad y el pensamiento fuera de lo convencional, permitiendo a los investigadores explorar su imaginación y habilidades narrativas de una manera única y divertida. Además, esta actividad promueve un ambiente de risas y alegría en el aula, creando un espacio donde el aprendizaje se convierte en un juego creativo.\n' +
      '\n' +
      'Es importante destacar que, aunque la excusa sea ficticia y cómica, también es fundamental que los estudiantes comprendan la importancia de la responsabilidad y la organización en el cumplimiento de las tareas escolares. Esta actividad busca ser un momento de desahogo humorístico, pero también una oportunidad para aprender sobre el poder de la imaginación y la expresión creativa.\n' +
      '\n' +
      'Esperamos que esta actividad, «La Tarea Mentirosa», no solo genere risas y diversión, sino que también inspire a los estudiantes a explorar nuevas formas de comunicación ya descubrir el placer de contar historias. ¡Estamos emocionados de ver las sorpresas y ocurrencias que nuestros estudiantes nos tienen preparados!\n' +
      '\n' +
      'Atentamente,\n' +
      '\n' +
      'Profesoras de cuarto de la sede B El Consuelo: Leyanis, Celmira, Mary y Claudia',
    slider: [{
      url: 'src/img/IMG_20230621_100147-1024x461.jpg',
    },
      {
        url: 'src/img/IMG_20230621_100339-edited-scaled.jpg',
      },
      {
        url: 'src/img/IMG_20230621_100335-edited-scaled.jpg',
      }

    ],
    videos: [
      {
        video: 'src/video/YouCut_20230701_230311805.mp4'
      }
    ]
  },
  {
    id: 8,

    title: '«El Insuperable y Divertido Juguetón: Mi Compañero de Aventuras» Mi juguete favorito.',
    text: 'En esta emocionante ejercicio, los estudiantes han traído a clase a su juguete favorito para compartirlo con sus compañeros. ¡Es una oportunidad única para conocer la magia y la historia que hay detrás de cada juguete especial!\n' +
      '\n' +
      'Cada estudiante tiene la oportunidad de describir su juguete, contar anécdotas emocionantes y compartir por qué este compañero de aventuras es tan especial para ellos. Desde ositos de peluche desgastados hasta coloridos sets de construcción, cada juguete tiene una historia y un lugar especial en el corazón de su dueño.\n' +
      '\n' +
      'Una vez que todos hayan compartido sus tesoros, los estudiantes se embarcarán en una apasionante actividad: crear su propio minilibro sobre su juguete favorito. Con imaginación y palabras, plasmarán las experiencias y vivencias que han tenido junto a este querido amigo de juegos.\n' +
      '\n' +
      'En este emocionante viaje literario, los estudiantes aprenderán a apreciar y valorar la importancia de sus juguetes favoritos, no solo como objetos, sino como compañeros que han estado a su lado en cada aventura. ¡Así que prepárense para descubrir la magia de los juguetes a través de las páginas de los maravillosos minilibros que estos talentosos estudiantes crearán! ¡Es hora de dar vida a la creatividad y a la nostalgia mientras escribimos sobre «Mi juguete favorito»!',

  },
  {
    id: 9,
    imagen: [{
      id: 1,
      img: 'src/img/El-municipio-en-el-pais-494x350.jpg',
    },
      {
        id: 2,
        img: 'src/img/IMG_20230519_082248-1024x461.jpg',

      },
      {
        id: 3,
        img: 'src/img/IMG_20230519_082205-461x1024.jpg',

      }
    ],
    title: '¡Explorando Mi Barrio: Un Mapa Lleno de Sorpresas y Vecinos Curiosos!». Cartografía de mi barrio del municipio de Girón.',
    text: 'La emocionante «Cartografía de mi Barrio» en el municipio de Girón! En esta actividad, los niños han creado coloridos carteles que son mucho más que simples mapas. Estas obras de arte son ventanas a sus mundos, donde nos muestran las historias de su barrio y nos destacan a los fascinantes iconos y vecinos que hacen de este lugar un rincón especial.\n' +
      '\n' +
      'El municipio de Girón ubicado en el país\n' +
      'Cada cartel es una representación y única y vibrante de la vida en el barrio, capturando los lugares favoritos de los niños, los rincones misteriosos y las aventuras diarias que los hacen sentir como en casa. A través de dibujos llenos de imaginación y palabras llenas de emoción, los niños nos guiarán por calles y plazas, compartiendo anécdotas y secretos que solo ellos conocen.\n' +
      '\n' +
      'Desde la tiendita de la esquina, donde compran golosinas, hasta la casa de «¡La Vecina Curiosa: Oídos afilados y lenguaje veloz!» o la casa del abuelo que siempre cuenta las mejores historias, cada icono y representa vecino un pedacito del corazón de estos barrios del sur de Girón.\n' +
      '\n' +
      'Así que prepárense para sumergirse en este maravilloso viaje cartográfico, donde los niños son nuestras guías entusiastas y nos enseñarán que cada lugar tiene su propia magia y que las historias más fascinantes están escondidas justo en nuestra propia comunidad. ¡Es hora de abrir los carteles y embarcarnos en una aventura única por la «Cartografía de mi Barrio»!',
    slider: [{
      url: 'src/img/IMG_20230519_082405-1024x461.jpg',
    },
      {
        url: 'src/img/IMG_20230519_082353-1024x461.jpg',
      },
      {
        url: 'src/img/IMG_20230519_082236-1024x461.jpg',
      },
      {
        url: 'src/img/IMG_20230519_082412-1024x461.jpg',
      }

    ],

  },
  {
    id: 10,
    imagen: [{
      id: 1,
      img: 'src/img/IMG_20230621_082247-461x1024.jpg',
    },
      {
        id: 2,
        img: 'src/img/IMG_20230621_082233-461x1024.jpg',

      },
      {
        id: 3,
        img: 'src/img/IMG_20230621_082319-461x1024.jpg',

      }
    ],
    title: 'Entre Sueños y Realidades: Las Páginas que Narran mi Mundo en ¡Así vivo yo!',
    text: '«¡Así vivo yo!», fascinante Actividad donde los estudiantes tienen la oportunidad de plasmar su historia a través de un minilibro! En este emocionante viaje, descubriremos las experiencias, sueños y vivencias de estos jóvenes talentos mientras nos sumergimos en las páginas llenas de creatividad y autenticidad.\n' +
      '\n' +
      'Cada mini libro es un reflejo único de la vida de estos estudiantes de los grados 4-1; 1-1, capturando momentos especiales, desafíos superados y momentos de alegría que los han moldeado en quienes son hoy. Desde pequeñas anécdotas hasta grandes logros, estas páginas están llenas de emociones y enseñanzas que nos conectarán con la esencia de cada autor.\n' +
      '\n' +
      'Prepárate para embarcarte en un viaje inolvidable, donde las palabras y las ilustraciones cobran vida para mostrarnos el mundo desde la perspectiva única de cada estudiante. «¡Así vivo yo!» es un tributo a la diversidad de experiencias y personalidades que hacen que cada estudiante sea especial!\n' +
      '\n' +
      'Acompáñanos en esta aventura literaria, donde descubriremos cómo estos talentosos jóvenes expresan su vida, sus sueños y esperanzas en el maravilloso formato de un mini libro. ¡Es hora de abrir estas pequeñas joyas y explorar los universos que hay dentro!',
    slider: [{
      url: 'src/img/IMG_20230621_082310-461x1024.jpg',
    },
      {
        url: 'src/img/IMG_20230621_082557-461x1024.jpg',
      },
      {
        url: 'src/img/IMG_20230621_082319-461x1024.jpg',
      }

    ],
  },
  {
    id: 11,

    title: '¡Bienvenidos a la página web «Pisquitas de Memoria: Conectando Generaciones»! En esta plataforma, te sumergirás en un emocionante viaje que combina la sabiduría de nuestros abuelos con la curiosidad e imaginación de los estudiantes de los grados: 4-1JM; 1-1JT de primaria Sede B El Consuelo ',
    text: '¿Qué son las Pisquitas de Memoria?\n' +
      '\n' +
      'Las «Pisquitas de Memoria» son pequeños destellos de recuerdos que surgen de manera espontánea en la mente de los abuelos. Son fragmentos fugaces de experiencias pasadas, imágenes, olores, sonidos, juguetes, mascotas y sensaciones táctiles que, aunque pueden ser breves, tienen un gran significado emocional y pueden evocar recuerdos significativos de nuestro pasado en estos estudiantes.\n' +
      '\n' +
      'El objetivo de este proyecto es crear un espacio de conexión y aprendizaje entre los abuelos y los estudiantes de 4-1 JM Y 1-1 JT de primaria, centrándonos en la transmisión intergeneracional y el poder de la memoria. Reconocemos que los abuelos son portadores de una vasta experiencia y conocimiento, mientras que los estudiantes están en una etapa de descubrimiento y ansias de aprender.\n' +
      '\n' +
      'Además de las actividades centradas en las «Pisquitas de Memoria», también comprendemos la importancia de fortalecer las habilidades de lectura y escritura de los estudiantes. En ese sentido, nuestro proyecto busca ofrecer un enfoque enriquecedor que combine la transmisión de conocimientos y experiencias de los abuelos con la mejora de las habilidades de Lectura, Escritura, Oralidad y Memorias (LEOM) en los estudiantes.\n' +
      '\n' +
      'Una forma de lograr esto es mediante la creación de un proyecto de escritura colaborativa en el que los estudiantes pueden trabajar junto a sus abuelos para escribir un libro. Este proceso no solo les permitirá desarrollar sus habilidades de escritura, sino también fomentará el vínculo intergeneracional y brindará a los abuelos la oportunidad de compartir sus historias y conocimientos de manera significativa.\n' +
      '\n' +
      'A través de la lectura, escritura, oralidad y memorias conjunta, los estudiantes podrán sumergirse en las experiencias de vida de sus abuelos, conocer distintos contextos históricos y culturales, y expandir su comprensión del mundo que los rodea. Estas actividades no solo tienen el potencial de mejorar las habilidades de lectura y escritura de los estudiantes, sino que también pueden aumentar su motivación y compromiso, lo que puede tener un impacto positivo en su desempeño académico, incluidas las pruebas de estado.\n' +
      '\n' +
      'En resumen, nuestro proyecto busca unir a los abuelos y estudiantes de cuarto y primer grado de primaria a través de la transmisión intergeneracional y el poder de la memoria. Al combinar actividades centradas en las «Pisquitas de Memoria» con proyectos de lectura , escritura, Oralidad colaborativa, esperamos fortalecer las habilidades de lectura y escritura de los estudiantes y brindarles una experiencia enriquecedora que impacte positivamente su aprendizaje y rendimiento académico.',

  },
  {
    id: 12,
    imagen: [{
      id: 1,
      img: 'src/img/segundapartedeSorbitosdememoriafoto1.jpg',
    },
      {
        id: 2,
        img: 'src/img/segundapartedeSorbitosdememoriafoto2.jpg',

      },
      {
        id: 3,
        img: 'src/img/segundapartedeSorbitosdememoriafoto3.jpg',

      },
      {
        id: 4,
        img: 'src/img/segundapartedeSorbitosdememoriafoto4.jpg',

      }
    ],
    title: '¡Bienvenidos de nuevo la segunda parte de "Sorbitos de Memoria" en la emisora comunitaria San Juan de Girón bajo la dirección de la historiadora Patricia Sandoval!',
    text: 'Hoy nuevamente nos sumergiremos en los valiosos recuerdos de los abuelos de los estudiantes del grado 4-1, donde los niños encuentran en sus abuelos un tesoro invaluable: la oportunidad de conocer su historia a través de relatos que viajan desde el pasado hasta el presente.\n' +
      'Así que, con una taza de nostalgia y otra de aprendizaje, nos disponemos a disfrutar de esta enriquecedora experiencia junto a los abuelos del grado 4-1. Acompáñenos en "Sorbitos de Memoria", donde el ayer cobra vida en el presente. ¡Comencemos!\n',
    youtube: 'OQK7zMZTXY0'
  },
  {
    id: 13,
    imagen: [{
      id: 1,
      img: 'src/img/WhatsApp Image 2023-08-06 at 11.32.58 AM.jpeg',
    },
      {
        id: 2,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.31.05 AM.jpeg',

      },
      {
        id: 3,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.31.05 AM (2).jpeg',

      },
      {
        id: 4,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.31.05 AM (1).jpeg',

      }, {
        id: 5,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.31.04 AM.jpeg',
      },
      {
        id: 6,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.31.04 AM (3).jpeg',

      },
      {
        id: 7,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.31.04 AM (2).jpeg',

      },
      {
        id: 8,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.31.04 AM (1).jpeg',

      },{
        id: 9,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.29.52 AM.jpeg',

      },
      {
        id: 10,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.29.51 AM.jpeg',

      },
      {
        id: 11,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.29.51 AM (4).jpeg',

      }, {
        id: 12,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.29.51 AM (3).jpeg',
      },
      {
        id: 13,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.29.51 AM (2).jpeg',

      },
      {
        id: 14,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.29.51 AM (1).jpeg',

      },
      {
        id: 15,
        img: 'src/img/WhatsApp Image 2023-08-06 at 11.29.50 AM.jpeg',

      }
    ],
    title: 'Creación del Fanzine',
    text: 'El objetivo es el de integrar a padres de familia, estudiantes y al profesor Juan Sebastián Peña quien nos enseña a la elaboración del fanzine; es una forma de publicación casera y artística que adopta la forma de pequeños libros. Además de fomentar la creatividad y expresión artística, el fanzine fortalece los lazos entre la comunidad educativa, convirtiéndose en un espacio de encuentro donde cada voz y cada historia son valoradas y celebradas, se promueve la libertad de expresión y la diversidad de contenido, convirtiéndose en un medio único y personal que celebra la autenticidad y la pasión por la creación. En estas páginas, las historias surgen tanto de las narraciones de los abuelos, la imaginación, juguetes favoritos, lugares especiales en el barrio, hasta las adorables mascotas, travesuras y cartas llenas de amor y agradecimiento a nuestros queridos taitas.',
  },
  {
    id: 14,
    imagen: [{
      id: 1,
      img: 'src/img/WhatsApp Image 2023-08-06 at 12.45.52 PM (1).jpeg',
    },
      {
        id: 2,
        img: 'src/img/WhatsApp Image 2023-08-06 at 12.45.52 PM (2).jpeg',

      },
      {
        id: 3,
        img: 'src/img/WhatsApp Image 2023-08-06 at 12.45.52 PM (3).jpeg',

      },
      {
        id: 4,
        img: 'src/img/WhatsApp Image 2023-08-06 at 12.45.52 PM.jpeg',

      },{
        id: 5,
        img: 'src/img/WhatsApp Image 2023-08-06 at 12.45.53 PM (1).jpeg',
      },
      {
        id: 6,
        img: 'src/img/WhatsApp Image 2023-08-06 at 12.45.53 PM (2).jpeg',

      },
      {
        id: 7,
        img: 'src/img/WhatsApp Image 2023-08-06 at 12.45.53 PM.jpeg',

      },
      {
        id: 8,
        img: 'src/img/WhatsApp Image 2023-08-06 at 12.45.54 PM (1).jpeg',

      },
      {
        id: 9,
        img: 'src/img/WhatsApp Image 2023-08-06 at 12.45.54 PM.jpeg',

      }
    ],
    title: 'Tecnología de Palabras: Un Viaje Digital hacia la Creatividad Literaria OLE',
    text: 'Un viaje hacia la Oralidad, lectura y escritura. Luego de plasmar sus mejores historias en papel, los estudiantes aprovechan las posibilidades tecnológicas para elevar sus historias a un nivel superior.\n' +
      'Enciendan lo portátiles, abran Word y déjense llevar por la imaginación. brindándoles la oportunidad de explorar la escritura desde una perspectiva más dinámica y versátil. utilizando herramientas de edición, corrección y diseño para darle forma a sus escritos.',
  },
]